...
export default function HomePage() {
  const { yamlName, pipelineNames, yamlIsValid, yamlError, setYamlName, setYamlContent, setPipelineNames, setYamlIsValid, setYamlError, setStatus } =
    usePipeline();
  ...
  return (
    <div className="grid gap-4 p-5">
      <section className="grid gap-3 rounded-md border border-slate-200 bg-white p-4">
        ...
        {!yamlIsValid && yamlError ? (
          <p className="text-xs text-amber-700">{yamlError}</p>
        ) : null}
